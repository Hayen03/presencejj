...
use crate::{extract::{ExtractError, excel::into_string, prog::extract_group_info_from_prog}, ui::{AppState, FilePoll, UIError, UpdateAction, screens::Desc}};
...
pub fn charger_de_prog(state: Arc<AppState>) -> crate::ui::actions::ActionResult {
	...
	let thread_handle: std::thread::JoinHandle<Result<(), UIError>> = std::thread::spawn(move || {
		let filepath = FilePoll::load("Sélectionner le fichier de présence".into())
			.with_filter("xlsx", &["xlsx"])
			.poll(state.clone());
		let filepath = if let Some(filepath) = filepath {
			filepath
		} else {
			return Err(UIError::CancelAction { desc: "Aucun fichier sélectionné".into() });
		};
	...
	Ok(UpdateAction::Push(Box::new(screen) as Box<dyn crate::ui::Screen>).one())
